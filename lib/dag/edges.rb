module Dag
  module Edges
    ...
    module EdgeInstanceMethods
      ...
      def links_to_source
        self.class.with_descendant_point(source, scoped_record_id)
      end

      # all links that start from the sink
      def links_from_sink
        self.class.with_ancestor_point(sink, scoped_record_id)
      end
      ...
      def rewire_crossing(above_leg, below_leg)
        ...
        if bridging_leg.nil?
          bridging_leg = self.class.new(self.class.conditions_for(source, sink, scoped_record_id))
      ...
      def wiring
        source = self.source
        sink = self.sink
        above_sources = links_to_source.map(&:source)
        below_sinks = links_from_sink.map(&:sink)
        above_bridging_legs = []
