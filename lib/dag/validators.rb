module Dag
  ...
  class CreateCorrectnessValidator < ActiveModel::Validator
    def validate(record)
      ...
      count = check_possible(record)
      record.errors.add(:base, "Cannot create a direct link with a count other than 0") if count == 1
      record.errors.add(:base, "Cannot create an indirect link with a count less than 1") if count == 2
    end
